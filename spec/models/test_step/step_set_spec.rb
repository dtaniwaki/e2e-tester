...
RSpec.describe TestStep::StepSet, type: :model do
  ...
  let(:test) { create :test, user: user, base_test_step_set: base_test }
  subject(:test_step) { build :step_set_step, test_step_set: test, shared_test_step_set: shared_test_step_set }
  describe 'validate :validate_accessibility' do
    context 'for unaccessible user' do
      ...
      context 'for copied test' do
        let(:base_test) { create :test, user: another_user, test_steps: build_list(:step_set_step, 1, shared_test_step_set: shared_test_step_set) }
        it 'succeeds' do
