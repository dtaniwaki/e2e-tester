...
RSpec.describe TestStepSet, type: :model do
  ...
  describe '#same_test_step_set?' do
    ...
    subject { create :test, base_test_step_set: base_test_step_set, title: 'foo', description: 'foo', browsers: browsers }
    ...
    context 'against the test step set' do
      let(:target) { create :shared_test_step_set, base_test_step_set: base_test_step_set, title: 'foo', description: 'foo' }
    ...
    context 'for other instance' do
      let(:target) { create :test, base_test_step_set: base_test_step_set, title: 'foo', description: 'foo', browsers: browsers }
      before do
      ...
      context 'test_step_set_id is different' do
        let(:target) { create :test, base_test_step_set: create(:test), title: 'foo', description: 'foo', browsers: browsers }
        it 'returns false' do
          expect(subject.same_test_step_set?(target)).to be false
        end
      end
